...
Router.map(function () {
    ...
    this.route('rss', { //we use "where" to signify this is a server-side route
        where:  'server',
        path:   '/feed.xml',
        action: function () {
            var feed = new RSS({
                title: "New Microscope Posts",
                description: "The latest posts from Microscope, the smallest (and best) news aggregator."
            });
    ...
    this.route('apiPosts', { //I think there is an issue with 'find()' that is causing this to not work right
        where: 'server',
        path: '/api/posts',
        action: function() {
